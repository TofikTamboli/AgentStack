...
export interface OptionWheelRef {
  selectPrevious: () => void;
  selectNext: () => void;
...
export const OptionWheel = forwardRef<OptionWheelRef, OptionWheelProps>(
  function OptionWheel(
    ...
    const playTick = useCallback(() => {
      const { soundUrl, soundVolume } = cfgRef.current as {
        soundUrl: string;
        soundVolume: number;
      };
    ...
    const runFrame = useCallback(
      (now: number) => {
        ...
        for (let i = 0; i < n; i++) {
          ...
          el.style.transform = `translate(${x.toFixed(
            2
          )}px, calc(${y.toFixed(2)}px - 50%)) rotate(${rot.toFixed(3)}deg)`;
    ...
    const applyTarget = useCallback(
      (value: number, snap: boolean) => {
        ...
        if (idx !== selectedRef.current) {
          ...
          const selectedLabel = cfg.items[idx] ?? "";
          onChangeRef.current?.(idx, selectedLabel);
          playTick();
        }
        startLoop();
      },
    ...
    // Expose OptionWheel imperative navigation methods
    ...
    useEffect(() => {
      if (selected !== undefined && Math.round(targetRef.current) !== selected) {
    ...
    return (
      <div
        ...
        style={{
          ["--ow-text-color" as string]: textColor,
          ["--ow-active-color" as string]: activeColor,
          ["--ow-font-size" as string]: `${fontSize}rem`,
          ["--ow-inset" as string]: `${inset}px`,
        }}
